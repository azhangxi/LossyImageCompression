...
public:
    ...
    * resulting rectangles. (Ties will be broken by taking the 
    * largest of the tied locations (rightmost, or bottommost, 
    * depending on splitting dimension). )
    * 
...
private:
   ...
   */
   ...
   void clearH(Node* &subRoot);

   Node* copy(const Node* subRoot);

   Node* pruneH(Node* subRoot, int tol);

   bool canBePrune(Node* subRoot, RGBAPixel &avg, int tol);

   bool dist(RGBAPixel first, RGBAPixel second, int  tol);

   void pruneSizeH(Node* subRoot, int & pSize, int  tol);
